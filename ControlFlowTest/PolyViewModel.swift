...
class PolyViewModel {
  ...
  init() {
    ...
    let dAngle:Angle = Angle(degrees:360.0 / Double(numCorners))
  ...
  private func rotate() {
    ...
    var moveToOrigin:[CGPoint] = Array(repeating: CGPoint(x:0.0, y:0.0), count: numCorners)
    for i in 0..<numCorners {
    ...
    var rotateAtOrigin:[CGPoint] = Array(repeating: CGPoint(x:0.0, y:0.0), count: numCorners)
    for i in 0..<numCorners {
